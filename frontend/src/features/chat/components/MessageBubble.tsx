  ...
  return (
    <article
      ...
      className={clsx(
        ...
        !isUser ? 'flex gap-3' : null,
      )}
    >
      {!isUser && (
        <div className="mt-0.5 hidden h-20 aspect-[816/1283] shrink-0 overflow-hidden rounded-md border border-slate-200 bg-slate-100 shadow-[0_4px_12px_rgba(16,185,129,0.18)] lg:block">
          <img src={avatarImage} alt="" className="h-full w-full object-cover" />
        </div>
      )}

      <div className={clsx('min-w-0', !isUser ? 'flex-1' : null)}>
        <header className="mb-2 flex items-center justify-between text-xs font-medium uppercase tracking-[0.08em] text-slate-500">
          <span>{isUser ? 'Você' : 'Lina'}</span>
